...
# ── Voice → Text via IBM Watson STT (primary) → Groq Whisper (fallback) ────────

async def transcribe_voice(audio_bytes: bytes, lang: str = "hi") -> str:
    """
    Route voice transcription through the provider abstraction layer.
    When AI_PROVIDER=ibm: tries IBM Watson Speech to Text first.
    Falls back to Groq Whisper on any IBM failure (incl. local DNS errors).
    """
    import asyncio
    from services import ai_provider

    # Run the blocking ai_provider call in a thread (it's sync)
    loop = asyncio.get_event_loop()
    try:
        result = await loop.run_in_executor(None, ai_provider.transcribe_audio, audio_bytes, lang)
        if result:
            print(f"[STT] Transcribed via provider ({os.getenv('AI_PROVIDER','aws')}): {len(result)} chars")
            return result
    except Exception as e:
        print(f"[STT] ai_provider transcription failed, falling back to Groq Whisper: {e}")
    ...
    try:
        ...
        print(f"[STT] Groq Whisper fallback used: {len(result)} chars")
        return result
    except Exception as e:
...
async def telegram_webhook(request: Request):
    ...
    # ── VOICE: IBM Watson STT → Groq Whisper fallback ───────────────────────
    if "voice" in message:
        ...
        try:
            ...
            text = await transcribe_voice(audio_bytes, lang=msg_language)
            if not text:
    ...
    try:
        ...
        lang = msg_language if msg_language != "auto" else (detect_language(text) or "en")
        llm_reply = get_llm_response(history, text, lang)
        cleaned_reply, _ = extract_options_tag(llm_reply)
        reply = cleaned_reply
