...
if not firebase_admin._apps:
    try:
        cred = credentials.Certificate(
            os.getenv("FIREBASE_CREDENTIALS_PATH", str((Path(__file__).parent.parent / "firebase-credentials.json").resolve()))
        )
...
async def whatsapp_webhook(request: Request):
    try:
        ...
        print(f"[WA-HOOK] Form parse error: {e}")
    ...
    print(f"[WA-HOOK] from={phone} body={body!r} num_media={num_media} media_type={media_type} media_url={media_url}")
    ...
    if num_media > 0 and media_url and "audio" in media_type:
        ...
        try:
            ...
            if len(audio_bytes) < 100:
                ...
                logger.warning("[AUDIO] Tiny response (%d bytes) from Twilio media URL", len(audio_bytes))
        except Exception as dl_err:
            print(f"[AUDIO] Download FAILED: {dl_err}")
            logger.error("[AUDIO] Download error: %s", dl_err)
        ...
        if transcript:
            ...
            print(f"[STT] Resolved: {transcript!r} -> {resolved!r}")
            body = resolved
            body_lower = body.lower().strip()
            ...
            heard_msgs = {
                ...
                "ml": f"🎤 കേട്ടു: _{transcript}_",
                "bn": f"🎤 শুনলাম: _{transcript}_",
                "as": f"🎤 শুনিলোঁ: _{transcript}_",
            }
            # We'll include heard message with next response
        else:
            print(f"[STT] Transcription returned empty for {len(audio_bytes)} bytes of audio")
            return twiml(m("error", lang))
