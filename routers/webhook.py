...
def _get_db():
    ...
    if not firebase_admin._apps:
        cred_path = os.getenv("FIREBASE_CREDENTIALS", str((Path(__file__).parent.parent / "firebase-credentials.json").resolve()))
...
def _send_whatsapp_screenshot(user, scheme, user_data, ref):
    ...
    try:
        ...
        from pathlib import Path
        static_dir = str((Path(__file__).parent.parent / "static").resolve())
